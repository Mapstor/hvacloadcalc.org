...
<KeyTakeaways
  items={[
    ...
    'Three target accuracy bands documented: ±5% on the 8 ACCA reference cases, ±3-5% versus Wrightsoft and Cool Calc on identical inputs, ±15% versus measured peak demand for 90%+ of real homes. Per-case results are being progressively published as the verified-runs data set is built out.',
    ...
    'Anyone can audit us: source formulas, reference cases, peer comparison method, and a contact for bug reports are public. Per-case verification data tables are being published progressively; the most recent verified runs are tracked in the calculator changelog and the corrections log.',
  ]}
/>
...
<Callout type="info" title="Validation data status (current as of last review)">
The accuracy targets described on this page (±5% on the 8 ACCA reference cases, ±3-5% versus peer software, ±15% versus measured peak demand) define what the calculator is verified against. The per-case data tables, including input snapshots, expected outputs, the calculator's output, and the delta, are being progressively published as the verified-runs data set is built out. Until each per-case table is published, the claims on this page should be read as documented verification targets with results available on request via the [corrections page](/corrections/). The [Manual J calculator changelog](/methodology/changelog/manual-j/) tracks every result-affecting change; reviewers wanting the most recent verification snapshot should reference the changelog and the corrections log together.
</Callout>
...
Verification target: within ±5% on all 8 cases. The per-case input snapshots, expected outputs, calculator outputs, and deltas are being published progressively as the verified-runs data set is built out; the [Manual J calculator changelog](/methodology/changelog/manual-j/) tracks each result-affecting code change, and the corrections log tracks any verification result that does not meet target.

This is the strongest baseline for verification because ACCA defined both inputs and expected outputs. There is no input ambiguity; the only question is whether the calculator produces the expected number. Reviewers wanting the most recent verification snapshot can reach the editorial team via the contact channel for the current data sheet.

Manual J reference cases are the foundational verification step. They confirm the calculator follows the methodology as ACCA defines it.
...
Target band: differences across implementations within ±5% on identical inputs, consistent with the inter-implementation variance documented in the academic Manual J comparison literature. Per-home comparison data tables are being progressively published as the verified-runs data set is built out; the published changelog tracks each result-affecting change.
...
Target sample: ~50 homes spanning climate zones, ages, and sizes; sampling is ongoing and the published data table is being built out progressively.

Target band: 90%+ of homes within ±15% agreement between calculated load and inferred peak demand, consistent with Manual J's inherent input-uncertainty floor documented in the building-science literature. Typical outlier sources documented during sampling:
